...
(function(){
  ...
  window.renderAtividadesPublic=function(){
    ...
    host.innerHTML='<div class="student-module-shell"><aside class="student-module-sidebar"><div class="side-label">MÓDULOS</div><div class="student-module-list">'+modules.map(m=>'<button class="student-module-btn '+(m.id===active.id?'active':'')+'" onclick="selectStudentActivityModule(\''+m.id+'\')"><strong>'+escapeHtml(m.title)+'</strong><span>'+(m.items||[]).length+' atividade(s) • Disponível</span></button>').join('')+'</div></aside><div class="student-module-main"><div class="student-module-top"><div><span class="eyebrow">MÓDULO SELECIONADO</span><h3>'+escapeHtml(active.title)+'</h3><p>Conteúdo publicado pelo professor.</p></div><span class="student-module-badge">'+(active.items||[]).length+' atividade(s)</span></div><div class="student-stage-list">'+(active.items||[]).map((it,i)=>'<article class="student-stage '+(it.thumbnail?'':'no-thumb')+'">'+(it.thumbnail?'<img src="'+escapeAttr(it.thumbnail)+'" class="content-thumb" alt="Thumbnail de '+escapeAttr(it.title||'Atividade')+'">':'')+'<div class="student-stage-head"><div class="student-stage-num">'+(i+1)+'</div><div style="flex:1"><h4>'+escapeHtml(it.title||('Atividade '+(i+1)))+'</h4><p>'+escapeHtml(it.desc||it.topic||'Atividade publicada pelo professor.')+'</p><div class="student-stage-meta">'+(it.topic?'<span class="student-tag">'+escapeHtml(it.topic)+'</span>':'')+(it.due?'<span class="student-tag">Entrega: '+escapeHtml(it.due)+'</span>':'')+'</div>'+(it.file?'<div style="margin-top:9px"><a href="'+(/^https?:\/\//.test(it.file)?escapeAttr(it.file):'#')+'" target="_blank" rel="noopener" class="resource-link">🔗 Abrir material</a></div>':'')+'</div></div></article>').join('')+'</div></div></div>';
  };
